...
(function (c2js) {
    ...
    function setAttrIfNotExists(el, attr, value) {
        c2(el).each(function () {
            if (!c2(this).attr(attr)) {
                c2(this).attr(attr, value);
            }
        });
    }
    ...
    class init {
        constructor(el, config) {
            ...
            this.ctrls = {
                ...
                stop: {
                    ...
                    media: {
                        'loadstart ended abort error': function (_e, inst, helpers) {
                ...
                mute: {
                    ...
                    media: {
                        volumechange: function (_e, inst, helpers) {
                ...
                fullscreen: {
                    ready: function (_e, inst, helpers) {
                        ...
                        FS_VAR.onChange((e) => {
                            if (inst.c2js === e.target) {
                                helpers.$all.data('fullscreen', FS_VAR.check());
                            }
                        });
                        FS_VAR.onError((e) => {
                            if (inst.c2js === e.target) {
                ...
                'time-seek': {
                    ...
                    ready: function (_e, _i, helpers) {
                        helpers.$all.each(function (_, el) {
                            setAttrIfNotExists(el, 'step', 0.1);
                            setAttrIfNotExists(el, 'max', 100);
                            c2(el).val(0);
                        });
                    },
                    events: {
                    ...
                    media: {
                        'timeupdate seeked': function (_e, inst, helpers) {
                ...
                'volume-seek': {
                    ...
                    media: {
                        'loadstart seeked volumechange': function (_e, inst, helpers) {
                            helpers.$all.each((_, el) => {
                                helpers.setSeek(el, inst.media);
                            });
                        }
                ...
                time: {
                    ...
                    media: {
                        timeupdate: function (_e, inst, helpers) {
                            helpers.$all.each((_, el) => {
                ...
                duration: {
                    media: {
                        durationchange: function (_e, inst, helpers) {
                            helpers.$all.each((_, el) => {
                ...
                loop: {
                    events: {
                        click: function (_e, inst) {
                            inst.media.loop = !inst.media.loop;
                            if (inst.media.loop) {
                                c2(this).data('loop', true);
                                return;
                            }
                            c2(this).data('loop', false);
                        }
                    }
                },
                speed: {
            ...
            this.$c2js = c2(el),
                this.c2js = el,
                this.$media = this.$c2js.find('video, audio'),
        ...
        addStatus(status) {
            this.$c2js.attr(c2js.APP_NAME, (`${this.status} ${status}`).trim());
        }
        rmStatus(rm) {
            let rmRegExp = new RegExp('\\s?(' + rm + ')');
        ...
        initControls() {
            setAttrIfNotExists(this.c2js, 'tabindex', -1);
    ...
    (function (c2) {
        class Query {
            ...
            on(events, fn) {
                events = events.split(' ');
                fn.$handler = function () { return fn.apply(this, arguments); };
            ...
            css(styleName, value) {
                ...
                let elem = this.get(0), view = elem.ownerDocument.defaultView;
